...
def _headed_env_enabled() -> bool:
    """Whether any platform requested a headed (visible) Chrome.

    兼容两代环境变量：历史 ``CHAOXING_HEADED``（全平台生效）与按平台标签的
    ``{PLATFORM}_HEADED``（由会话名 ``{platform}-chrome-N`` 推导，如
    ZHIHUISHU_HEADED）。
    """
    if os.environ.get("CHAOXING_HEADED", "0") == "1":
        return True
    session = _get_active_session()
    tag = session.split("-chrome")[0].upper() if "-chrome" in session else ""
    return bool(tag) and os.environ.get(f"{tag}_HEADED", "0") == "1"
...
def _rebuild_session(session: str) -> None:
    ...
    Mirrors the platform auth open invocation (no --headless flag: headless is
    the daemon default; --headed only in headed mode). Orphan sweep and profile
    resolution are platform-agnostic helpers in core.browser.orphans.
    """
    from .orphans import kill_orphaned_chrome, profile_dir_for_session
    cli = cfg("playwright_cli", "playwright-cli.cmd")
    try:
    ...
    kill_orphaned_chrome(profile_dir_for_session(session, idx))
    profile_dir = profile_dir_for_session(session, idx)
    cmd = [cli, f"-s={session}", "open", "--browser=chrome", "--persistent",
           f"--profile={profile_dir}", "about:blank"]
    if _headed_env_enabled():
        cmd.append("--headed")
    try:
        result = subprocess.run(
...
def _default_profile_dir(idx: int):
    """Legacy flat chaoxing profile path (kept for backward compatibility)."""
