...
from core.constants import LOG_DIR, SHUTDOWN_FLAG

# ── Log file naming ─────────────────────────────────────────────
# Daily log files are named ``<prefix>_YYYYMMDD.log``. The default keeps the
# historical chaoxing name; a platform package may switch it at import time
# (e.g. zhihuishu sets "zhihuishu") via set_log_file_prefix().
LOG_FILE_PREFIX = "chaoxing"


def set_log_file_prefix(prefix: str) -> None:
    """Override the daily/error log filename prefix for this process."""
...
def log(msg: str, level: str = "INFO"):
    ...
    try:
        ...
        log_file = log_dir / f"{LOG_FILE_PREFIX}_{time.strftime('%Y%m%d')}.log"
        with _log_lock:
...
def log_exception(context: str, exc: Exception = None):
    ...
    try:
        ...
        if not _log_dir_created:
            log_dir.mkdir(parents=True, exist_ok=True)
            _log_dir_created = True
        err_file = log_dir / f"{LOG_FILE_PREFIX}_errors_{time.strftime('%Y%m%d')}.log"
        with _log_lock:
