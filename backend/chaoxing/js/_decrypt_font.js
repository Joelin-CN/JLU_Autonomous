...
window._cxMd5=function(s){return hex(md51(s));};})();
...
// Wrapper: expose font-cxsecret decryption for call from page.evaluate
// tableJsonStr = JSON.stringify of the table.json mapping (hash → codepoint)
// Returns {ok: true, decrypted: N} or {ok: false, reason: "..."}
window._cxDecryptFont = function(doc, tableJsonStr) {
    const table = JSON.parse(tableJsonStr);
    const styles = doc.querySelectorAll("style");
    let tip;
    for (let i = 0; i < styles.length; i++) {
        if (styles[i].textContent && styles[i].textContent.includes("font-cxsecret")) {
            tip = styles[i];
            break;
        }
    }
    if (!tip) return {ok: false, reason: "no-cxsecret-style"};

    const m = tip.textContent.match(/base64,([\w\W]+?)'/);
    if (!m) return {ok: false, reason: "no-base64-font"};
    ...
    const font = new Font_1(buffer);
    const match = {};
    for (let i = 19968; i < 40870; i++) {
        const glyph = font.codeToGlyph(i);
        if (!glyph) continue;
        const path = font.glyphToPath(glyph);
        const hash = window._cxMd5(JSON.stringify(path)).slice(24);
        match[i] = table[hash];
    }
    ...
    return {ok: true, decrypted: elements.length};
};
