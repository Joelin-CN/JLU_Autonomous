...
## 附录 D：M2 实施期新发现（2026-09-12，有头 Chrome 实测）

1. **登录 Cookie 为会话级**：扫码成功后登录态只在当前浏览器进程存活，Chrome 重启即失效（profile 持久化无效）。与超星（Cookie 落盘）关键差异——智慧树每任务需现场建立登录态（扫码/密码+滑块），M3 评估 `context.storageState()` 导出恢复。
2. **playwright-cli 的 `--headed` 只能作为 `open` 子命令选项**；作为全局前缀会令 click/fill 等命令整体失败（"Unknown option: --headed"）。此为继承自超星引擎的隐性 bug（CHAOXING_HEADED=1 时点击全灭），已在 core 引擎修复。
3. **章节树懒加载**：`ul.list` 位于 `el-scrollbar`，只渲染可视区（实测锁定课仅见 1 章/3 节）→ M3 需滚动加载后解析全树。
4. 登录页 Tab 的 DOM id 为内部编号（账号登录=#tab-1.1 / 学号登录=#tab-2 / 工号登录=#tab-2.1 / 扫码=#tab-3），不能写死，须按文本发现。
5. 已登录状态下智慧树主页仍存在指向 login.zhihuishu.com 的链接 → 登录态判定须在课程列表页（onlineweb）进行，或按链接可见文本恰为「登录」收紧。
